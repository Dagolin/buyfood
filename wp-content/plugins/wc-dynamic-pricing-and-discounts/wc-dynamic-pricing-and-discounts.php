...
if (!class_exists('RP_WCDPD')) {
    ...
    class RP_WCDPD
    {
        ...
        public function __construct()
        {
            ...
            // Load helper class
            include_once RP_WCDPD_PLUGIN_PATH . 'includes/classes/libraries/rightpress-helper.class.php';
            include_once RP_WCDPD_PLUGIN_PATH . 'includes/classes/libraries/rightpress-wc-legacy.class.php';
        ...
        public function replace_visible_prices_cart($item_price, $cart_item, $cart_item_key)
        {
            ...
            $price = get_option('woocommerce_tax_display_cart') == 'excl' ? RightPress_WC_Legacy::product_get_price_excluding_tax($cart_item['data']) : RightPress_WC_Legacy::product_get_price_including_tax($cart_item['data']);
            ...
            if ($the_coupon->is_valid() && !$woocommerce->cart->has_discount($coupon_code)) {
                ...
                if ($woocommerce->cart->applied_coupons) {
                    foreach ($woocommerce->cart->applied_coupons as $code) {
                        ...
                        else if (RightPress_Helper::wc_version_gte('3.0') && $coupon->individual_use) {
                            return false;
                        }
                    }
                }
        ...
        public function maybe_add_virtual_coupon($unknown_param, $coupon_code)
        {
            if ($coupon_code == $this->get_fake_coupon_code()) {
                $coupon = array(
                    ...
                    'individual_use'                => (RightPress_Helper::wc_version_gte('3.0') ? false : 'no'),
                    'product_ids'                   => array(),
        ...
        public function apply_pricing_adjustment($cart_item_key, $adjustment)
        {
            ...
            $woocommerce->cart->cart_contents[$cart_item_key]['rp_wcdpd'] = array(
                'original_price'    => get_option('woocommerce_tax_display_cart') == 'excl' ? RightPress_WC_Legacy::product_get_price_excluding_tax($woocommerce->cart->cart_contents[$cart_item_key]['data']) : RightPress_WC_Legacy::product_get_price_including_tax($woocommerce->cart->cart_contents[$cart_item_key]['data']),
                'log'               => $adjustment['log'],
            );

            // Actually adjust price in cart
            RightPress_WC_Legacy::product_set_price($woocommerce->cart->cart_contents[$cart_item_key]['data'], $adjustment['price']);
        }
        ...
        public function product_page_pricing_table()
        {
            ...
            $product_id = RP_WCDPD::product_get_absolute_id($product);
            ...
            if (isset($this->opt['pricing']['sets']) && count($this->opt['pricing']['sets'])) {
                foreach ($this->opt['pricing']['sets'] as $rule_key => $rule) {
                    ...
                    if ($rule['method'] == 'quantity' && $validated_rule = RP_WCDPD_Pricing::validate_rule($rule)) {
                        ...
                        if ($validated_rule['selection_method'] == 'categories_include' && count(array_intersect($this->get_product_categories($product_id), $validated_rule['categories'])) > 0 && $this->user_matches_rule($validated_rule)) {
                            $selected_rule = $validated_rule;
                            break;
                        }
                        if ($validated_rule['selection_method'] == 'categories_exclude' && count(array_intersect($this->get_product_categories($product_id), $validated_rule['categories'])) == 0 && $this->user_matches_rule($validated_rule)) {
                            $selected_rule = $validated_rule;
                            break;
                        }
                        if ($validated_rule['selection_method'] == 'products_include' && in_array($product_id, $validated_rule['products']) && $this->user_matches_rule($validated_rule)) {
                            $selected_rule = $validated_rule;
                            break;
                        }
                        if ($validated_rule['selection_method'] == 'products_exclude' && !in_array($product_id, $validated_rule['products']) && $this->user_matches_rule($validated_rule)) {
                            $selected_rule = $validated_rule;
                            break;
                        }
                    }
            ...
            if (is_array($selected_rule)) {
                ...
                if ($selected_rule['method'] == 'quantity' && in_array($this->opt['settings']['display_table'], array('modal', 'inline')) && isset($selected_rule['pricing'])) {
                    ...
                    if (RightPress_WC_Legacy::product_get_type($product) == 'variable' && !empty($product_variations)) {
                    ...
                    if ($multiprice_variable_product) {
                        ...
                        if (RightPress_WC_Legacy::product_get_type($product) && !empty($product_variations)) {
        ...
        /**
         * Get product absolute id (parent id for variations, id for simple products)
         *
         * @access public
         * @param object $product
         * @return int
