...
class YouTubePrefs
{
    ...
    public static function get_productLink_html($thumb)
    {
        ...
        while ( $loop->have_posts() ) : $loop->the_post();
            ...
            if ($escId == $video_id) {
                return '<div class="epyt-gallery-title"><p>前往：<a href="' . $product->get_permalink() . '" target="_blank">' . $product->get_title() . '</a></p></div>';
            }
    ...
    public static function get_thumbnail_html($thumb, $options)
    {
        ...
        if ($options->style != 'carousel' && !empty($thumb->title))
        {
            ...
            $code .= self::get_productLink_html($thumb);
        }
        else
        {
...
//            $new_options[self::$opt_hl] = $_hl;
        ...
        <style type="text/css">
            ...
            #box_collapse_grid {font-weight: bold; padding: 0px 10px; <?php echo $all[self::$opt_gallery_collapse_grid] ? 'display: block;' : 'display: none;' ?>}
